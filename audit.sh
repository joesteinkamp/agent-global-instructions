...
case "$MODE" in
  timeline) tail -n "$N" "$LOG" | jq -r "$fmt";;
  follow)   tail -n 0 -f "$LOG" | jq -r --unbuffered "$fmt";;
  stats)
    echo "Log: $LOG"
    echo "Entries: $(wc -l < "$LOG" | tr -d ' ')"
    echo ""; echo "By harness:"; jq -r '.tool // "?"'      "$LOG" | sort | uniq -c | sort -rn
    echo ""; echo "By tool:";    jq -r '.tool_name // "?"' "$LOG" | sort | uniq -c | sort -rn
    echo ""; echo "By event:";   jq -r '.event // "?"'     "$LOG" | sort | uniq -c | sort -rn;;
esac
